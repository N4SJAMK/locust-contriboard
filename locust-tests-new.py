...
class TeamboardTasks(TaskSet):
	...
	def set_activity(self):
		...
		board = self.boards[random.choice(self.boards.keys())]
		with self.client.put('api/boards/' + board['id'] + '/setactivity',
									json = {
									...
									timeout=MAX_WAIT_TIME_SECONDS,
									name='Board Set Activity',
									catch_response=True) as respose:
			if respose.status_code != 200:
				respose.failure("Set activity failed, code: " + str(respose.status_code))
